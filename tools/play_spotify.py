...
def play_spotify(search_query):
    try:
        ...
        refined_query_json = refined_query_json.strip('```json').strip('`').replace('""', '"').strip()
        ...
        if 'albums' in result and 'items' in result['albums'] and len(result['albums']['items']) > 0:
            album = result['albums']['items'][0]
            context_uri = album['uri']
        elif 'tracks' in result and 'items' in result['tracks'] and len(result['tracks']['items']) > 0:
            track = result['tracks']['items'][0]
            context_uri = track['uri']
        elif 'artists' in result and 'items' in result['artists'] and len(result['artists']['items']) > 0:
        ...
        if context_uri is None:
            os.system(f'{SPEAK_COMMAND} "Could not find any music for the following command: {search_query}"')
        else:
            if device_id:
        ...
        print(e)
        traceback.print_exc()
        os.system(f'{SPEAK_COMMAND} "An error occurred while trying to play the music."')
...
def control_spotify(command):
    try:
        ...
        print(e)
        traceback.print_exc()
        os.system(f'{SPEAK_COMMAND} "An error occurred while trying to control the music."')
